...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}24}]:} 0.0009064288519737423
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}28}]:} 0.9038
\end{Verbatim}
...
p-value of 0.90 means that there is a 90\% probability of finding the
observed value (or a more extreme value) if the null hypothesis is true.
Since we previously established that we would only tolerate a Type I
error rate of 5\% or less, a 0.90 p-value means we cannot reject the
null hypothesis. By not rejecting the null hypothesis, we say that there
is no evidence that there is a true difference between new and old
pages.
...
\end{enumerate}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}30}]:} \PY{n}{sm}\PY{o}{.}\PY{n}{stats}\PY{o}{.}\PY{n}{proportions\PYZus{}ztest}\PY{p}{(}\PY{p}{[}\PY{n}{convert\PYZus{}old}\PY{p}{,} \PY{n}{convert\PYZus{}new}\PY{p}{]}\PY{p}{,} \PY{p}{[}\PY{n}{n\PYZus{}old}\PY{p}{,} \PY{n}{n\PYZus{}new}\PY{p}{]}\PY{p}{,} \PY{n}{alternative}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{smaller}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
\end{Verbatim}


\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}30}]:} (1.3109241984234394, 0.9050583127590245)
\end{Verbatim}
...
in parts j. and k. Namely, the p-value of 0.91, when compared to an
alpha of 0.05, leads us to not reject the null hypothesis, which means
we have no evidence to indicate that the proportion of conversion in the
new page is different from the proporsion of conversion in the old page.
